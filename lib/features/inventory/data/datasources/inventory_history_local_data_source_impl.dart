  ...
  Future<List<InventoryHistory>> getHistories() async {
    final query = '''
      SELECT 
        ...
        (
          SELECT oq.${OpeningQuantitiesTable().countUnits}
          FROM ${OpeningQuantitiesTable().tableName} oq
          WHERE oq.${OpeningQuantitiesTable().inventoryId} = ito.${InventoryTransactionsOrdersTable().inventoryId}
          ORDER BY oq.${OpeningQuantitiesTable().createdAt} DESC
          LIMIT 1
        ) AS opening_quantity
