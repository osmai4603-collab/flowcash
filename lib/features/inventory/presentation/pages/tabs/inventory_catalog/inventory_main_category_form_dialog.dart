...
class InventoryMainCategoryFormDialog extends StatefulWidget {
  const InventoryMainCategoryFormDialog({super.key});

  @override
  State<InventoryMainCategoryFormDialog> createState() =>
      _InventoryMainCategoryFormDialogState();
}

class _InventoryMainCategoryFormDialogState
    extends State<InventoryMainCategoryFormDialog> {
  ...
  // Loaded data lists
  List<MainCategoryEntity> _mainCategories = [];
  ...
  Future<void> _loadFormData() async {
    try {
      final mainCategoriesRes = await sl<GetAllMainCategoriesUseCase>().call();
      ...
      if (mounted) {
        setState(() {
          mainCategoriesRes.fold((_) => null, (list) => _mainCategories = list);
  ...
  Future<void> _onMainCategorySelected(MainCategoryEntity? mainCategory) async {
    ...
    try {
      final matchedSubcategoryIds = _subcategories
          .where((s) => s.mainCategoryId == mainCategory.id)
          .map((s) => s.id)
          .toSet();

      if (matchedSubcategoryIds.isEmpty) {
        setState(() {
          _resolvedCategories = [];
      ...
      final resolved = _categories
          .where((c) => matchedSubcategoryIds.contains(c.subcategoryId))
          .toList();
      ...
      fluent.displayInfoBar(
        ...
        builder: (context, close) => fluent.InfoBar(
          ...
          content: const fluent.Text(
            'الرجاء اختيار الصنف الرئيسي والمستودع وتحديد جميع الحسابات المالية',
          ),
        ),
      );
      return;
    ...
    if (_filteredResolvedCategories.isEmpty) {
      fluent.displayInfoBar(
        ...
        builder: (context, close) => fluent.InfoBar(
          title: const fluent.Text('تنبيه'),
          content: const fluent.Text(
            'جميع الأصناف التابعة لهذا الصنف الرئيسي موجودة مسبقاً في هذا المستودع.',
          ),
        ),
      );
      return;
          ...
          children: [
            ...
            Row(
              children: [
                ...
                const fluent.Text(
                  'إضافة مخزون رئيسي جديد (بالصنف الرئيسي)',
                  style: TextStyle(
                    fontSize: 20,
            ...
            Column(
              ...
              children: [
                ...
                // 1. Basic properties Row (Main Category Selection)
                fluent.InfoLabel(
                  label: 'الصنف الرئيسي',
                  child: ComboBoxForm<MainCategoryEntity>(
                    ...
                    labelMenu: (category) => category.name,
                    labelString: (category) => category.name,
                    itemsBuilder: (value) {
                    ...
                    onSelectedItem: (category) {
                      _onMainCategorySelected(category);
                    },
                    onChanged: (value) {
                      if (_selectedMainCategory != null &&
                          _selectedMainCategory!.name != value) {
                        _onMainCategorySelected(null);
                      }
                    },
                    validator: (_) =>
                        _selectedMainCategory == null ? 'مطلوب اختيار الصنف الرئيسي' : null,
                  ),
                ),
