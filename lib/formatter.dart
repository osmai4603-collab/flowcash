...
import 'package:flutter/material.dart' show TextField, debugPrint;
...
class CreditCardFormatter extends NumberInputFormatter {
  ...
  final int lengthWord;

  CreditCardFormatter({this.separator = ' ', this.lengthWord = 3});

  @override
  String formatPattern(String digits) {
    StringBuffer buffer = StringBuffer();
    int offset = 0;
    int count = min(lengthWord, digits.length);
    final length = digits.length;
    for (; count <= length; count += min(lengthWord, max(1, length - count))) {
...
class MoneyFormatter extends NumberInputFormatter {
  static final RegExp _digitOnlyRegex = RegExp(r'[1-9]+[.0-9]?');
  static final FilteringTextInputFormatter _digitOnlyFormatter =
      FilteringTextInputFormatter.allow(_digitOnlyRegex);

  final String separator = ',';
  final int lengthWord = 3;

  MoneyFormatter();

  @override
  String formatPattern(String digits) {
    debugPrint('Hello: $digits');
    final numbers = digits.contains('.') ? digits.split('.').first : digits;
    StringBuffer buffer = StringBuffer();
    int offset = 0;
    int count = min(lengthWord, numbers.length);
    final length = numbers.length;
    for (; count <= length; count += min(lengthWord, max(1, length - count))) {
      buffer.write(numbers.substring(offset, count));
      if (count < length) {
        buffer.write(separator);
      }
      offset = count;
    ...
    return buffer.toString() +
        (digits.contains('.') ? digits.split('.').last : '');
  }
